...
Clone this repository into the project that should use the plugin:

```bash
mkdir -p plugins
git clone https://github.com/<owner>/codex-heartbeat-plugin.git plugins/codex-heartbeat
```

Then ask Codex from that workspace:

```text
Use project-heartbeat to set up heartbeats for this repo.
```

Codex can read the plugin skill from `plugins/codex-heartbeat/skills/project-heartbeat/SKILL.md` and use the renderer at `plugins/codex-heartbeat/scripts/render_heartbeats.py`.

### Option 2: Register In A Local Marketplace

If you maintain a local Codex plugin marketplace, add an entry that points at the cloned plugin folder:
...
After the plugin is available in Codex, install or enable `Codex Heartbeat`, then run:
